<template>
  <div>
    ...
    <el-row type="flex" justify="space-between" class="mb-5">
      ...
      <el-col class="colored-cards turquoise-colored-cards" :class="{ 'filter-none': configure.markShow}" :span="5" @click.native="switchModular('markShow')">
        <i class="el-icon-ice-tea"></i> 唛标
      </el-col>
    </el-row>
    <el-row>
      <el-col class="mt-8" v-if="configure.baseShow"><Base /></el-col>
      <el-col v-if="configure.contactShow"><Contacts :isEdit="false" /></el-col>
      <el-col v-if="configure.bankAccountShow"><BankAccounts :isEdit="false" /></el-col>
      <el-col class="mt-8" v-if="configure.markShow"><Mark :isEdit="false" /></el-col>
    </el-row>
  </div>
</template>
